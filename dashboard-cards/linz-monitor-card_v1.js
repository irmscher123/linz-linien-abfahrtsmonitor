/* ---------------------------------------------------------
   LinzAG Monitor – V1 - Maxi
   --------------------------------------------------------- */

const LINE_COLORS_V1 = { 
...
/* --- EDITOR --- */
class LinzMonitorCardEditorV1 extends HTMLElement {
  ...
  render() {
    ...
    this.innerHTML = `
      <div class="card-config" style="padding:10px;">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:10px;">
          <label style="display:flex; align-items:center; gap:8px; background:#1a1a1a; border:1px solid #333; padding:10px; border-radius:6px; cursor:pointer;"><input id="show_info" type="checkbox" ${showInfo ? 'checked' : ''} style="transform:scale(1.2);"><span style="font-weight:700;">Info-Zeile</span></label>
          <label style="display:flex; align-items:center; gap:8px; background:#1a1a1a; border:1px solid #333; padding:10px; border-radius:6px; cursor:pointer;"><input id="badge_round" type="checkbox" ${badgeRound ? 'checked' : ''} style="transform:scale(1.2);"><span style="font-weight:700;">Badges rund</span></label>
        </div>
      </div>
    `;
...
if (!customElements.get("linz-monitor-card-editor-v1")) { customElements.define("linz-monitor-card-editor-v1", LinzMonitorCardEditorV1); }

/* --- CARD --- */
class LinzMonitorCardV1 extends HTMLElement {
  static getConfigElement() { return document.createElement("linz-monitor-card-editor-v1"); }
  constructor() { super(); this._gone_mem = new Map(); }

  setConfig(config) {
    this._config = { 
      entity: "sensor.linz_ag_monitor",
      anzahl: 7, sortierung: "echtzeit", stop_name_override: "", filter: "", font_family: "",
      show_info: true, badge_round: true, ...config
    };
    if (this._config.font_family) loadGoogleFontV1(this._config.font_family);
  }
  ...
  render(state, departures) {
    ...
    visibleRows.forEach((d) => {
      ...
      const isStandard = STANDARD_ROUTES_V1[cleanL]?.includes(d.direction);
      let lineT = d.line;
      if (!isStandard && STANDARD_ROUTES_V1[cleanL]) {
      ...
      let styledInfoHtml = "";
      if (isCancelled) {
         // ALARM: Fahrt fällt aus (Rot/Weiß)
         styledInfoHtml = `
      ...
      if (!rowEl) {
        ...
        tempRow.innerHTML = `
          <div class="line-badge" style="background:${LINE_COLORS_V1[cleanL] || "#444"}">${lineT}</div>
          <div class="main-body">
        ...
        if (badgeEl) {
          ...
          badgeEl.style.background = LINE_COLORS_V1[cleanL] || "#444";
        }
      }
...
if (!customElements.get("linz-monitor-card_v1")) {
  customElements.define("linz-monitor-card_v1", LinzMonitorCardV1);
}

window.customCards = window.customCards || [];
window.customCards.push({ 
  type: "linz-monitor-card_v1", 
  name: "Linz AG Monitor - V1 - Maxi", 
  description: "LinzAG Abfahrtsmonitor (maxi)",
  preview: true 
});
